...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if(data){
    ...
    if(!tokenRequested){
        const d = await walletRepo.requestPartnerWithdrawalToken(request);
        if(!d.data){
         return redirect("/partners/wallet")
         }
        tokenRequested = true;
...
export default function SetWithdrawalPin() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-white p-6 font-sans relative">
      <Link to="/partners/wallet" className="absolute top-4 right-4 text-gray-500 hover:text-gray-800"><X size={24} /></Link>
