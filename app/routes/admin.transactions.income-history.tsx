  ...
  if (walletsResponse.data?.length) {
    for (const _wallet of walletsResponse.data) {
      ...
      if (wallet_id_param && wallet_id_param === _wallet._id) {
        const pagedLedgers = await walletRepo.getOrganizationLedgersForWallet(request, {
...
export async function action({ request }: ActionFunctionArgs) {
    ...
    const ledgersResp = await walletRepo.getOrganizationLedgersForWallet(request, query);
    ...
    const walletsResp = await walletRepo.getOrganizationWallets(request);
    let wallets: WalletWithLedger[] = [];
    if (walletsResp.data?.length) {
      ...
      for (const _wallet of walletsResp.data) {
        if (_wallet._id === walletResp.data?._id) {
          ...
          const paged = await walletRepo.getOrganizationLedgersForWallet(request, { wallet_id: _wallet._id, page_size: 10 });
