...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const walletsResponse = await walletRepo.getOrganizationWallets(cookieHeader);
  ...
  if (walletsResponse.data?.length) {
    for (const _wallet of walletsResponse.data) {
      ...
      if (wallet_id_param && wallet_id_param === _wallet._id) {
        const pagedLedgers = await walletRepo.getOrganizationLedgersForWallet(cookieHeader, {
...
export async function action({ request }: ActionFunctionArgs) {
    ...
    const ledgersResp = await walletRepo.getOrganizationLedgersForWallet(cookieHeader, query);
    ...
    if (walletsResp.data?.length) {
      ...
      for (const _wallet of walletsResp.data) {
        if (_wallet._id === walletResp.data?._id) {
          ...
          const paged = await walletRepo.getOrganizationLedgersForWallet(cookieHeader, { wallet_id: _wallet._id, page_size: 10 });
