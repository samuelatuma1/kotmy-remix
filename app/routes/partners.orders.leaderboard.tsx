...
type LeaderboardLoaderData = {
  leaderboard: IPaginatedResponse<PartnerOrdersLeaderboardSearchResponse>;
...
function emptyPaginatedLeaderboard(pageSize = 20): IPaginatedResponse<PartnerOrdersLeaderboardSearchResponse> {
  return {
    current_page: 1,
    total_pages: 0,
    total_items: 0,
...
function LeaderboardRow({ entry }: { entry: PartnerOrdersLeaderboardSearchResponse }) {
  return (
    <article className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
      <div className="flex items-center gap-4">
        ...
        <div className="shrink-0 rounded-full bg-slate-950 px-4 py-2 text-sm font-bold text-white">
          {entry.currency} {" "} {entry.amount?.toLocaleString('en')}
        </div>
      </div>
    </article>
  );
          ...
          <div className="max-w-3xl space-y-4">
            ...
            <div className="space-y-3">
              ...
              <p className="max-w-2xl text-base leading-7 text-slate-600 sm:text-lg">
               Your top earners for the selected time frame
              </p>
            </div>
          </div>
        </div>
