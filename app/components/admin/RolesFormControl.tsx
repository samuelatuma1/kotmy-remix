...
export default function RolesFormControl({ roles: roles, defaultRoles: defaultRoles, ...props }: { roles: string[], defaultRoles?: string[] } & React.ComponentProps<'fieldset'>) {
    ...
    function resetFieldset(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
        e.currentTarget.form?.roles.forEach((item: HTMLInputElement) => {
            item.checked = item.defaultChecked
        })
    }
    function labelize(_role: string) {
    ...
    return (
        <fieldset ref={fieldset} {...props} className='p-2 sm:p-4 rounded-lg bg-transparent border hover:border-primary sm:col-span-2'>
            ...
            <div className={cn("grid sm:grid-cols-3 gap-6 mt-4 sm:mx-3", { 'hidden': !open })}>
                {roles.map(role => (
                    <FormControl key={role} as='input' type='checkbox' name='role' value={role} className='w-min'
                        defaultChecked={defaultRoles?.includes(role)} labelText={labelize(role)}
