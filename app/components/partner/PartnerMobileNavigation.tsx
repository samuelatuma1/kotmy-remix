...
export default function PartnerMobileNavigation({ show, onClose }: { show: boolean, onClose: () => void }) {
    ...
    useEffect(() => {
        ...
        if(!currentUser) navigate(`/login?redirectTo=${encodeURIComponent(path)}`);
        setUser(currentUser);
